...
function create() {
   // Game menu overlay
       menuOverlay = this.add.rectangle(400, 300, 400, 200, 0x000000, 0.8).setDepth(10);
    startButton = this.add.text(400, 300, 'START', { font: '48px Arial', fill: '#fff', backgroundColor: '#333', padding: { x: 32, y: 16 } })
        .setOrigin(0.5)
        .setInteractive()
        .setDepth(11);
    ...
    pauseOverlay = this.add.rectangle(400, 300, 400, 200, 0x000000, 0.7).setDepth(21).setVisible(false);
    resumeButton = this.add.text(400, 270, 'RESUME', { font: '32px Arial', fill: '#fff', backgroundColor: '#333', padding: { x: 24, y: 8 } })
        .setOrigin(0.5)
        .setInteractive()
        .setDepth(22)
        .setVisible(false);
    restartButton = this.add.text(400, 330, 'RESTART', { font: '32px Arial', fill: '#fff', backgroundColor: '#333', padding: { x: 24, y: 8 } })
        .setOrigin(0.5)
        .setInteractive()
        .setDepth(22)
        .setVisible(false);
    ...
    healthText = this.add.text(10, 40, 'Health: 100', { font: '20px Arial', fill: '#fff' });
...
function update(time, delta) {
    ...
    enemies.children.each(function(enemy) {
        if (enemy.active && player.active) {
            let speed = (enemy.enemyType && enemy.enemyType.speed) ? enemy.enemyType.speed : 100;
...
function spawnEnemy() {
    ...
    let x, y;
    if (edge === 0) { x = 0; y = Phaser.Math.Between(0, 960); }
